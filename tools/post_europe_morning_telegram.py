...
def offer_caption(offer: dict, test: bool = False) -> str:
    ...
    lines = [
        f"🚘 <b>{title}</b>",
        "<blockquote><b>💰 ПОД КЛЮЧ В РФ</b>",
        f"<b>{turnkey_price}</b>",
        f"<i>Курс ЦБ РФ · {rate_date}</i></blockquote>",
        "",
        "<b>Характеристики</b>",
        f"🛣 Пробег: <b>{mileage}</b>",
        f"⚙️ Двигатель: <b>{engine}</b>",
        f"🏁 Мощность: <b>{power}</b>",
        ...
        f'☎️ <b>{CONTACT_PHONE_DISPLAY}</b>  ·  <a href="{CONTACT_TELEGRAM_URL}">Telegram</a>',
    ]
    caption = "\n".join(lines)
