...
function summarize(result) {
  ...
  const total = roundRub(result.totalWithCarRub);
  return {
    turnkey_price_rub: complete ? total : null,
    turnkey_partial_price_rub: complete ? null : total,
    ...
    turnkey_calculation_complete: complete,
    turnkey_calculation_warnings: result.warnings || [],
    turnkey_calculation_errors: result.errors || [],
  };
}

function buildInput(car, rates) {
  const engineType = mapEngineType(car);
  const carRub = Number(car.price_krw || 0) > 0
    ? Number(car.price_krw) * rates.KRW
    : Number(car.price || 0) * rates.USD;
  const roadRub = KOREA_ROAD_EUR * rates.EUR;

  return {
    vehicleType: 'passenger_car',
    importerType: engineType === 'electric' ? 'legal_stp' : 'physical_ets',
    price: Math.round((carRub + roadRub) * 100) / 100,
    currency: 'RUB',
    deliveryToBorder: 0,
    insuranceToBorder: 0,
...
const updated = sourceCars.map((car) => {
  const result = calculateCustoms(buildInput(car, rates));
...
console.log(`Updated ${updated.length} Korea cars`);
...
console.log(`Road cost: ${KOREA_ROAD_EUR} EUR`);
